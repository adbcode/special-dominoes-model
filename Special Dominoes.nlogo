to setup
  clear-all
  ;; create dominoes
  ask n-of (dominoes-count - starter-dominoes) patches [
    set pcolor red
  ]
  ;; starter dominoes
  ask n-of starter-dominoes patches with [pcolor != red] [
    set pcolor green
  ]
  reset-ticks
end
...
to go
  ;; check if there are dominoes to still fall
  if count patches with [pcolor = green] = 0 [stop]
  ask patches with [pcolor = green]
  [
    if propagation = "4-way"
    [
...
"" ""
PENS
"standing" 1.0 0 -2674135 true "" "plot count patches with [pcolor = red]"
"fallen" 1.0 0 -7500403 true "" "plot count patches with [pcolor = gray]"
"active" 1.0 0 -10899396 true "" "plot count patches with [pcolor = green]"
